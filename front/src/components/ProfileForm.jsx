...
const ProfileForm = ({ user }) => {
	...
	useEffect(() => {
		change && axios.put("/api/change", { login, password: confirmedPassword }).then(() => {
			alert("Готово.");
	...
	const profileFormInput = (field, labelData, placeholder, value = "", type = "text", isRequired = true) => (
		<label key={field}>
	...
	const disabledCondition = !(enteredPassword === confirmedPassword && enteredPassword !== "" && confirmedPassword !== "");
			...
			<div className="profile_form__subcontainer">
				...
				<form onSubmit={onSubmit}>
					...
					<section>
						...
						<div>
							{[["name", "Имя", "Введите имя", inputs.name], ["surname", "Фамилия", "Введите фамилию", inputs.surname], ["middle_name", "Отчество", "Введите отчество", inputs.middle_name], ["id", "Идентификационный номер", "Введите идентификационный номер", inputs.id], ["login", "Логин", "Введите логин", inputs.login]].map(el => profileFormInput(...el))}
						</div>
					</section>
					<hr />
					<section>
						...
						<div>
							{[["email", "Адрес электронной почты", "agsr@mail.ru", inputs.email, "email", false], ["tel", "Мобильный номер", "+375 29 123 44 55", inputs.tel, "tel", false]].map(el => profileFormInput(...el))}
						</div>
					</section>
					<hr />
					<section>
